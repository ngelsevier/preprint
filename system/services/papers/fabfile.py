...
def build(aws_account_id, aws_ecr_hostname, version, commit_hash, parent_image_version):
    ...
    build_paper_events_replication_scheduler()

    build_paper_replication_scheduler()
...
@task
def build_paper_events_replication_scheduler():
...
@task
def build_paper_replication_scheduler():
    with lcd(os.path.join(get_fabric_file_directory_path(), 'infrastructure/paper-scheduler')):
        local('rm -rf build')
        local('mkdir build')
        ...
        with lcd('build'):
            ...
            local('find . -type f -name "*.pyc" -exec rm {} +')
            local('find . -exec touch --date="1970-01-01" {} +')
            local('zip -X -r ../paper-scheduler.zip *')
...
def configure_test_environment():
    ...
    in_fabfile_directory(lambda: migrate_database(fabfile_directory_path, 'papers', dry_run=False))
    in_fabfile_directory(lambda: migrate_database(fabfile_directory_path, 'papers_integration_tests', dry_run=False))
