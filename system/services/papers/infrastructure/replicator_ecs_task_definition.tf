resource aws_ecs_task_definition replicator {
  ...
  network_mode = "bridge"
  task_role_arn = "${data.terraform_remote_state.permissions.papers_replicator_iam_role_arn}"
}

data template_file replicator_container_definitions {
  template = "${file("replicator_container_definitions.json.tpl")}"
  ...
  vars {
    ...
    environment_variables_json_array = "${format("[ %s ]", join(", ", list(
      ...
      jsonencode(map("name", "EVENTS_FEED_MAX_PAGE_REQUEST_RETRIES", "value", var.events_feed_max_page_request_retries)),
