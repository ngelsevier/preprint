resource aws_cloudwatch_metric_alarm papers-database-oldest-replication-slot-lag-alarm {
  ...
  dimensions {
    DBInstanceIdentifier = "${var.product}-${var.environment}-${aws_db_instance.papers.name}"
  }
}
