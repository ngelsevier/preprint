...
def build(aws_account_id, aws_ecr_hostname, version, commit_hash, parent_image_version):
    ...
    build_healthcheck_scheduler()

    with lcd(get_fabric_file_directory_path()):
        local('mkdir -p build/contract-tests')
        local('fab -f website/fabfile.py build')
        local('fab -f website/fabfile.py package:../build/contract-tests/old-platform-contract-tests.tgz')
...
def watch_assets():
    ...
    with lcd(service_directory_path):
        local('gradle -Pwatch=true compileCss')

@task
def build_healthcheck_scheduler():
    with lcd(os.path.join(get_fabric_file_directory_path(), 'infrastructure/healthcheck-scheduler')):
        local('rm -rf build')
        local('mkdir build')
        ...
        with lcd('build'):
            ...
            local('zip -X -r ../healthcheck-scheduler.zip *')