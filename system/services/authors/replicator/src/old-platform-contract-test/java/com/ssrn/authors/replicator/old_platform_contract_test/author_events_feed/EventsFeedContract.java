...
import static com.ssrn.authors.replicator.old_platform_contract_test.author_events_feed.matcher.DateTimeWithTimeZoneMatcher.dateWithTimeZone;
import static com.ssrn.authors.replicator.old_platform_contract_test.author_events_feed.matcher.StringParseableAsTimestampMatcher.aDateTimeStringWithFormat;
import static com.ssrn.authors.replicator.old_platform_contract_test.author_events_feed.matcher.ValueNotNullMatcher.notNull;
...
public class EventsFeedContract extends SsrnOldPlatformContractTest {
    ...
    private final String authorsEventsFeedEntrypointUri = ssrnAbsoluteUrl("/rest/authors/events");
    ...
    public void expectFeedPageRequestToRespondWithOKStatus() {
        // Given, When
        Response response = httpClient().get(authorsEventsFeedEntrypointUri, headers(acceptApplicationJsonHeader, authorizationHeader));
    ...
    public void expectFeedPageRequestToRespondWithUnauthorizedWhenNoCredentialsSent() {
        // Given, When
        Response response = httpClient().get(authorsEventsFeedEntrypointUri, headers(acceptApplicationJsonHeader));
    ...
    public void expectFeedPageRequestToRespondWithUnauthorizedWhenInvalidCredentialsSent() {
        ...
        // When
        Response response = httpClient().get(authorsEventsFeedEntrypointUri, headers(acceptApplicationJsonHeader, invalidAuthorizationHeader));
    ...
    public void expectEntrypointToSupportConditionalGetRequests() {
        // Given
        Response response = httpClient().get(authorsEventsFeedEntrypointUri, headers(
        ...
        // When
        Response responseToIfNoneMatchRequest = httpClient().get(authorsEventsFeedEntrypointUri, headers(
                acceptApplicationJsonHeader,
                authorizationHeader,
                acceptEncodingHeader,
        ...
        // When
        Response responseToIfNoneMatchRequestWithDifferentEtag = httpClient().get(authorsEventsFeedEntrypointUri, headers(
                acceptApplicationJsonHeader,
                authorizationHeader,
                acceptEncodingHeader,
    ...
    public void expectWorkingPageCanonicalUrlToSupportConditionalGetRequests() {
        ...
        Response entrypointResponse = httpClient().get(
                authorsEventsFeedEntrypointUri,
    ...
    public void expectOldestPageToHaveNoPreviousArchiveLink() {
        ...
        Response response = httpClient().get(
                authorsEventsFeedEntrypointUri,
                String.format("/%s", Configuration.ssrn().firstAuthorsEventFeedPageId()),
                headers(acceptApplicationJsonHeader, authorizationHeader));
        String responseBody = response.readEntity(String.class);
    ...
    public void expectRequestForNonExistentPageToReturnNotFoundStatus() {
        // Given, When
        Response response = httpClient().get(
                authorsEventsFeedEntrypointUri, "/non-existent-page",
                headers(acceptApplicationJsonHeader, authorizationHeader));
    ...
    public void expectEntrypointUrlResponseToContainSelfLinkPointingToEntrypointUrl() {
        ...
        assertThat(responseBody, hasJsonPath("$._links.self.href", is(equalTo(authorsEventsFeedEntrypointUri))));
    }
    ...
    public void expectEventTimestampToBeUtcTimeOfEvent() {
        ...
        browser()
                ...
                .enterPublicDisplayNameTo(String.format("First %s Name", new Random().nextInt(9999)), "Last Name")
                .submitUpdates();

        // Then
        MatcherAssert.assertThat(() -> {
                    Event authorNameChangedEvent = ssrnApi().authorEventsStream().getNewEventsAfter(newestEventIdBeforeAuthorNameChanged, ssrnWebsite().accountId()).get(0);
                    return parseToDate(authorNameChangedEvent.getTimestamp());
                },
    ...
    public void expectEntityVersionToIncreaseConsecutivelyPerEntity() {
        // Given
        String author1FirstName = String.format("Test %d", new Random().nextInt(9999));
        String author1LastName = "User 1";
        ...
        String newestEventIdBeforeAuthor1NameChanged = ssrnApi().authorEventsStream().getNewestEvent().getId();

        browser()
                .click(ssrnWebsite().navigationBar().profileDropDown(ssrnWebsite().accountId()).personalInfoLink())
                .enterPublicDisplayNameTo(author1FirstName, author1LastName)
                .submitUpdates();

        assertThat(() -> ssrnApi().authorEventsStream().getNewEventsAfter(newestEventIdBeforeAuthor1NameChanged, ssrnWebsite().accountId()),
                eventuallySatisfies(hasSize(1)).within(20, SECONDS, checkingEvery(100, MILLISECONDS)));

        int author1EntityVersion = ssrnApi().authorEventsStream()
                .getNewEventsAfter(newestEventIdBeforeAuthor1NameChanged, ssrnWebsite().accountId())
                .get(0)
                .getEntityVersion();

        String author2FirstName = String.format("Test %d", new Random().nextInt(9999));
        String author2LastName = "User 2";

        browser()
                .visit(ssrnWebsite().loginPage())
                .logInAs(ssrnWebsite().thirdAuthorAccountUsername(), ssrnWebsite().thirdAuthorAccountPassword());

        String newestEventIdBeforeAuthor2NameChanged = ssrnApi().authorEventsStream().getNewestEvent().getId();

        browser()
                .click(ssrnWebsite().navigationBar().profileDropDown(ssrnWebsite().thirdAdditionalAuthorAccountId()).personalInfoLink())
                .enterPublicDisplayNameTo(author2FirstName, author2LastName)
                .submitUpdates();

        assertThat(() -> ssrnApi().authorEventsStream().getNewEventsAfter(newestEventIdBeforeAuthor2NameChanged, ssrnWebsite().thirdAdditionalAuthorAccountId()),
                eventuallySatisfies(hasSize(1)).within(20, SECONDS, checkingEvery(100, MILLISECONDS)));

        int author2EntityVersion = ssrnApi().authorEventsStream()
                .getNewEventsAfter(newestEventIdBeforeAuthor2NameChanged, ssrnWebsite().thirdAdditionalAuthorAccountId())
                .get(0)
                .getEntityVersion();

        // When
        ssrnWebsite().personalInformationPage(ssrnWebsite().thirdAdditionalAuthorAccountId()).loadedIn(browser(), true)
                .enterPublicDisplayNameTo(author2FirstName + " Updated", author2LastName)
                .submitUpdates();
        ...
        browser()
                .click(ssrnWebsite().navigationBar().profileDropDown(ssrnWebsite().accountId()).personalInfoLink())
                .enterPublicDisplayNameTo(author1FirstName + " Updated", author1LastName)
                .submitUpdates();

        assertThat(() -> ssrnApi().authorEventsStream().getNewEventsAfter(newestEventIdBeforeAuthor1NameChanged, ssrnWebsite().accountId()),
                eventuallySatisfies(hasSize(2)).within(20, SECONDS, checkingEvery(100, MILLISECONDS)));

        assertThat(ssrnApi().authorEventsStream()
                .getNewEventsAfter(newestEventIdBeforeAuthor1NameChanged, ssrnWebsite().accountId())
                .get(1)
                .getEntityVersion(), CoreMatchers.is(CoreMatchers.equalTo(author1EntityVersion + 1)));

        // Then
        assertThat(ssrnApi().authorEventsStream()
                .getNewEventsAfter(newestEventIdBeforeAuthor2NameChanged, ssrnWebsite()
                        .thirdAdditionalAuthorAccountId())
                .get(1)
                .getEntityVersion(), CoreMatchers.is(CoreMatchers.equalTo(author2EntityVersion + 1)));
    }
