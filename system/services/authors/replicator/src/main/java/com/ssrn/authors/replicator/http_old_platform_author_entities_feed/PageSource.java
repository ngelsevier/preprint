...
class PageSource {
    ...
    private static final String ENTITIES_FEED_PATH = "/rest/authors";
    ...
    Page getEntrypointPage() {
        return getPageStartingAfterAuthorId(null);
    }

    Page getPageStartingAfterAuthorId(Integer authorId) {
        UriBuilder uri = UriBuilder.fromPath(baseUrl)
                .path(ENTITIES_FEED_PATH);

        if (authorId != null) {
            uri.queryParam("afterId", authorId);
        }
