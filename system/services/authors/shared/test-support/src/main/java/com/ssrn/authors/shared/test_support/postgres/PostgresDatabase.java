...
public class PostgresDatabase implements AutoCloseable {
    ...
    private static final String DOCKER_CONTAINER_NAME = "authors_postgres_1";

    private final String eventsPublisherUsername;
    private final String eventsPublisherPassword;
    ...
    PostgresDatabase(String host, int port, String databaseName, String eventsPublisherUsername, String eventsPublisherPassword, String replicatorUsername, String replicatorPassword, String masterUsername, String masterPassword) {
        this.host = host;
        this.port = port;
        this.eventsPublisherUsername = eventsPublisherUsername;
        this.eventsPublisherPassword = eventsPublisherPassword;
    ...
    public String getEventsPublisherUsername() {
        return eventsPublisherUsername;
    }

    public String getEventsPublisherPassword() {
        return eventsPublisherPassword;
    }
    ...
    public void truncateTables() {
        try {
            truncateTableNamed("author", getConnection());
    ...
    public DateTime getLastUpdatedTimeForAuthor(Author author) {
        try {
            PreparedStatement preparedStatement = getConnection()
                    .prepareStatement("select last_updated from author where id = ?");
            preparedStatement.setString(1, author.getId());

            ResultSet resultSet = preparedStatement.executeQuery();

            if (!resultSet.next()) {
                throw new RuntimeException(String.format("Expected to find author in database with id '%s'", author.getId()));
            }
