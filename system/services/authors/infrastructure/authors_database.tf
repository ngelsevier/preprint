resource aws_route53_record authors_database {
  zone_id = "${data.terraform_remote_state.vpc.internal_route53_hosted_zone_id}"
  name = "authors-database"
  ttl = "60"
  type = "CNAME"
  records = [
    "${aws_db_instance.authors.address}"
  ]
}

resource aws_db_instance authors {
  ...
  db_subnet_group_name = "${aws_db_subnet_group.authors.id}"
  apply_immediately = true
  parameter_group_name = "${aws_db_parameter_group.authors.id}"
...
resource aws_db_parameter_group authors {
  name = "${var.product}-${var.environment}-${var.service_name}"
  family = "postgres9.6"
...
resource aws_db_subnet_group authors {
  name = "${var.product}-${var.environment}-${var.service_name}"
...
resource aws_lambda_function authors_database_heartbeat {
  function_name = "${var.product}-${var.environment}-${var.service_name}-database-heartbeat"
  handler = "heartbeat.lambda_handler"
  role = "${data.terraform_remote_state.permissions.authors_database_heartbeat_lambda_executor_iam_role_arn}"
  runtime = "python2.7"
...
resource aws_cloudwatch_event_rule authors_database_heartbeat_trigger {
  name = "${var.product}-${var.environment}-${var.service_name}-database-heartbeat-trigger"
  schedule_expression = "rate(1 minute)"
}

resource aws_cloudwatch_event_target authors_database_heartbeat_trigger_target {
  arn = "${aws_lambda_function.authors_database_heartbeat.arn}"
  rule = "${aws_cloudwatch_event_rule.authors_database_heartbeat_trigger.name}"
}

resource aws_lambda_permission authors_database_heartbeat_trigger_permission {
  action = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.authors_database_heartbeat.function_name}"
