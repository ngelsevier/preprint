...
class KinesisPapersStream implements PapersStream {

    private final Worker.Builder workerBuilder;
    private Worker worker;
    private boolean logIndividualPapers;

    KinesisPapersStream(String kclApplicationName, String kinesisStreamName, SimulatedEnvironmentConfiguration simulatedEnvironmentConfiguration, boolean logIndividualPapers) {
    ...
    @Override
    public void onPapersReceived(Consumer<List<Paper>> papersConsumer) {
        worker = workerBuilder.recordProcessorFactory(() -> new PaperRecordProcessor(papersConsumer, this.logIndividualPapers)).build();
        worker.run();
    }
