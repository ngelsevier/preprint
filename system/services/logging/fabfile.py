...
@task
def build_logging_janitor():
    with lcd(os.path.join(get_fabric_file_directory_path(), 'infrastructure/logging-janitor')):
        local('rm -rf build')
        local('mkdir build')

        with lcd('build'):
            local('cp ../logging-janitor.py .')
            local('pip install requests -t .')
            local('find . -type f -name "*.pyc" -exec rm {} +')
            local('find . -exec touch --date="1970-01-01" {} +')
            local('zip -X -r ../logging-janitor.zip *')
