package com.ssrn.test.support.old_platform_contract_test;
...
import com.ssrn.test.support.standalone_test_runner.AutoScanningTestSuite;
import com.ssrn.test.support.standalone_test_runner.ConsoleOutputListener;
...
public class TestRunner {
    ...
    public static void main(String[] args) {
        ...
        Configuration.addOptionsTo(options);

        CommandLine commandLine = parseCommandLineArgumentsFor(options, args);

        if (commandLine.hasOption("help")) {
            HelpFormatter helpFormatter = new HelpFormatter();
            helpFormatter.printHelp(150, "java -jar path/to/contract-tests.jar [options]", null, options, null);
            return;
        }

        Configuration.overrideIfSpecifiedIn(commandLine);
