module qa_deployment_agent {
  source = "./modules/automation_agent"

  agent_name = "qa-deployment-agent"
  ami_id = "${module.foundation.default_ami_id}"
  ansible_playbook_file_content_blobs = "${list(data.template_file.qa_deployment_agent_ansible_playbook_file_content.rendered)}"
  ...
  iam_instance_profile_name = "${data.terraform_remote_state.permissions.qa_deployment_agent_iam_instance_profile_name}"
...
data template_file qa_deployment_agent_ansible_playbook_file_content {
  ...
  vars {
    ...
    agent_environments = "${join(",", var.qa_deployment_agent_environments)}"
