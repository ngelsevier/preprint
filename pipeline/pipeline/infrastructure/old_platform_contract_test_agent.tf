module old_platform_contract_test_agent {
  source = "./modules/automation_agent"

  agent_name = "old-platform-contract-test-agent"
  ami_id = "${module.foundation.default_ami_id}"
  ansible_playbook_file_content_blobs = "${list(data.template_file.old_platform_contract_test_agent_ansible_playbook_file_content.rendered)}"
  ansible_requirements_file_content_blobs = "${list(data.template_file.old_platform_contract_test_agent_ansible_requirements_file_content.rendered)}"
...
data template_file old_platform_contract_test_agent_ansible_requirements_file_content {
  template = "${file("${path.module}/files/old_platform_contract_test_agent_ansible_requirements.yml.tpl")}"
...
data template_file old_platform_contract_test_agent_ansible_playbook_file_content {
  template = "${file("${path.module}/files/old_platform_contract_test_agent_ansible_playbook.yml.tpl")}"
  ...
  vars {
    ...
    agent_environments = "preprod-old-platform-contract-testing"
    agent_resources = ""
