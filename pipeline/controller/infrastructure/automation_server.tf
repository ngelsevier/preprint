...
resource aws_instance automation_server {
  ...
  iam_instance_profile = "${data.terraform_remote_state.permissions.controller_automation_server_iam_instance_profile_name}"
...
data template_file automation_server_ansible_playbook_file_content {
  ...
  vars {
    ...
    automation_server_backup_s3_key_prefix = "controller-automation-server"
    automation_server_encrypted_keystore_content = "${var.automation_server_encrypted_keystore_content}"
    automation_server_comma_separated_users_list = "${join(",", var.automation_server_users)}"
