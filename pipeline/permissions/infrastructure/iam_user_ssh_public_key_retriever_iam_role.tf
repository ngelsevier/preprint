resource aws_iam_role iam_user_ssh_public_key_retriever {
  name = "ssrn_pipeline.${var.environment}.iam-user-ssh-public-key-retriever"
...
resource aws_iam_policy iam_user_ssh_public_key_retrieval {
  name = "ssrn_pipeline.${var.environment}.iam_user_ssh_public_key_retrieval"
  path = "/"
...
data aws_iam_policy_document iam_user_ssh_public_key_retrieval {
  ...
  statement = {
    ...
    resources = [
      "arn:aws:iam::*:group/ssrn_pipeline.${var.environment}.ssh_users",
      "arn:aws:iam::*:group/ssrn_pipeline_controller.${var.environment}.ssh_users"
    ]
  }
}